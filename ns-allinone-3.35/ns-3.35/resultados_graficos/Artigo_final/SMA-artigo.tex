...
\author{Gabriel de Sousa\inst{1}, Gabriel Felix de Lima\inst{1}, Victor André de Moraes\inst{1}\\
Thiago Masera Tokarski\inst{1}, Gustavo Lopes Dezan\inst{1}}

\address{CIC -- Universidade de Brasília (UnB)\\
  DF -- Brazil\\
  \email{211010332@aluno.unb.br, 211056000@aluno.unb.br, 211026664@aluno.unb.br,}\\
  \email{190096063@aluno.unb.br, 202033463@aluno.unb.br}
}
...
\begin{abstract}
This work explores the feasibility of a decentralized cloud storage network through an agent-based simulation model. A simulation was developed in Python using the \texttt{asyncio} library to model interactions among three main actors: Storage Buyers, Providers, and an intermediate Broker. The model incorporates realistic dynamics such as finite storage capacity, time-limited contracts, network latency, and multiple failure sources. The simulation was executed across multiple scalability scenarios and showed that, even with simple rule-based agents, the architecture is functional and economically viable. The quantitative analysis covered economies ranging from 26 to 183 agents, processing up to 4,500 transactions and demonstrating success rates between 9.7\% and 15.7\%, financial volumes from \$894 to \$25,289, and throughput of up to 1.92 TPS. The results validate the conceptual model and establish a solid foundation for the future development of AI-powered agents.
\end{abstract}
     
\begin{resumo}
Este trabalho explora a viabilidade de uma rede de armazenamento em nuvem descentralizada por meio de um modelo de simulação baseado em agentes. Foi desenvolvida uma simulação em Python, utilizando a biblioteca \texttt{asyncio}, para modelar as interações entre três atores principais: Compradores de armazenamento, Provedores e um Broker intermediário. O modelo incorpora dinâmicas realistas como capacidade finita de armazenamento, contratos com duração limitada, latência de rede e múltiplas fontes de falha. A simulação executada em múltiplos cenários de escalabilidade revelou que, mesmo com agentes baseados em regras simples, a arquitetura é funcional e economicamente viável. A análise quantitativa abrangeu desde economias pequenas com 26 agentes até economias massivas com 183 agentes, processando até 4.500 transações e demonstrando taxas de sucesso entre 9,7\% e 15,7\%, volumes financeiros de \$894 a \$25.289, e throughput de até 1,92 TPS. Os resultados validam o modelo conceitual e estabelecem uma base sólida para o futuro desenvolvimento de agentes dotados de inteligência artificial.
\end{resumo}
...
\hline
\textbf{Problema} & A predominância de provedores centralizados de serviços em nuvem limita a flexibilidade, aumenta os custos e reduz a transparência na alocação de recursos computacionais. \\
\hline
...
\hline
\textbf{Hipótese} & É possível projetar e simular um sistema multiagentes que negocie contratos de nuvem em blockchain, otimizando a alocação e o gerenciamento de recursos. \\
\hline
...
\hline
\textbf{Método} & Pesquisa Bibliográfica, Modelagem e Desenvolvimento, Simulação Computacional, Avaliação e Validação. \\
\hline
\textbf{Classificação} & Natureza: Aplicada; Objetivos: Exploratória; Abordagem: Quantitativa; Técnica: Simulação. \\
\hline
\end{tabular}
\end{table}
...
\section{Descrição do Sistema Completo}
...
\subsection{Modelo de Metas e Dependências do Sistema}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{cloud_storage_analysis.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{performance_analysis_detailed.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{multi_agent_cloud_storage_complete_analysis.png}
...
A análise de diferentes durações de simulação indicou convergência estatística adequada para períodos superiores a 120 segundos, validando a metodologia utilizada e sugerindo que simulações mais longas poderiam revelar padrões de comportamento emergente adicionais.
...
\begin{enumerate}
\item AMAZON WEB SERVICES. \textit{What is Cloud Storage?} Disponível em: \textit{https://aws.amazon.com/what-is/cloud-storage/}. Acesso em: 26 jul. 2025.

\item CAMEL-AI TEAM. \textit{CAMEL: Communicative Agents for "Mind" Exploration of Large Scale Language Model Society}. Disponível em: \textit{https://github.com/camel-ai/camel}. Acesso em: 26 jul. 2025.

\item GERVAIS, Arthur et al. \textit{Bitcoin-Simulator}. Disponível em: \textit{https://github.com/arthurgervais/Bitcoin-Simulator}. Acesso em: 26 jul. 2025.

\item RUSSELL, Stuart J.; NORVIG, Peter. \textit{Artificial Intelligence: A Modern Approach}. 4. ed. Prentice Hall, 2020.

\item WOOLDRIDGE, Michael. \textit{An Introduction to MultiAgent Systems}. 2. ed. John Wiley \& Sons, 2009. ISBN: 978-0-470-51946-2.
\end{enumerate}
