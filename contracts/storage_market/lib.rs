...
pub mod storage_market {
    ...
    pub struct ProviderProfile {
        /// Optional off-chain metadata hash (e.g., IPFS CID)
        pub metadata_hash: Option<String>,
        /// Whether this account is registered as provider
        pub active: bool,
    }
    ...
    impl StorageMarket {
        ...
        /// Design minimalista: registra o provedor com apenas um hash opcional de metadados off-chain.
        #[ink(message)]
        ...
        /// Requisição de armazenamento: cria um acordo contendo apenas as informações essenciais.
        #[ink(message)]
        ...
        /// Aceite do comprador. Quando ambos aceitam, o contrato sinaliza 'Agreed' (pagamento virtual autorizado).
        #[ink(message)]
        pub fn accept_buyer(&mut self, deal_id: u128) -> bool {
            ...
            match self.deals.get(deal_id) {
                Some(mut d) => {
                    ...
                    if d.buyer_accepted && d.provider_accepted {
                        d.state = DealState::Agreed;
                        self.env().emit_event(DealAgreed { deal_id });
                    }
        ...
        /// Aceite do provedor. Quando ambos aceitam, o contrato sinaliza 'Agreed' (pagamento virtual autorizado).
        #[ink(message)]
        ...
        /// Conclusão: o comprador confirma que o armazenamento foi realizado. Marca como Completed.
        #[ink(message)]
        ...
        /// Consulta: obter estado de um acordo.
        #[ink(message)]
        pub fn get_deal(&self, deal_id: u128) -> Option<StorageDeal> {
            self.deals.get(deal_id)
        }

        /// Consulta: verificar se um provedor está ativo.
        #[ink(message)]
        pub fn is_provider_active(&self, account: AccountId) -> bool {
            self.providers.get(account).map(|p: ProviderProfile| p.active).unwrap_or(false)
        }
    }
    ...
    mod tests {
        ...
        fn provider_registration_and_flow() {
            ...
            // Provider registers
            set_caller(provider);
            assert!(sm.register_provider(Some(String::from("QmProviderMeta"))));
            assert!(sm.is_provider_active(provider));

            // Buyer requests storage
            set_caller(buyer);
            let deal_id = sm.request_storage(provider, String::from("QmFileCid123"));
            let d = sm.get_deal(deal_id).expect("deal exists");
            assert_eq!(d.state, DealState::Proposed);
            assert_eq!(d.buyer, buyer);
            assert_eq!(d.provider, provider);

            // Provider accepts
            set_caller(provider);
            assert!(sm.accept_provider(deal_id));
            let d = sm.get_deal(deal_id).unwrap();
            assert!(d.provider_accepted);
            assert_eq!(d.state, DealState::Proposed); // only provider accepted so far

            // Buyer accepts -> becomes Agreed (virtual payment authorized)
            set_caller(buyer);
            assert!(sm.accept_buyer(deal_id));
            let d = sm.get_deal(deal_id).unwrap();
            assert!(d.buyer_accepted);
            assert_eq!(d.state, DealState::Agreed);

            // Buyer completes -> Completed
            assert!(sm.complete_storage(deal_id));
            let d = sm.get_deal(deal_id).unwrap();
        ...
        fn only_parties_can_accept_and_complete() {
            ...
            let mut sm = StorageMarket::new();
            test::set_caller::<ink::env::DefaultEnvironment>(provider);
            assert!(sm.register_provider(None));

            test::set_caller::<ink::env::DefaultEnvironment>(buyer);
            let deal_id = sm.request_storage(provider, String::from("QmCID"));

            // Other cannot accept or complete
            test::set_caller::<ink::env::DefaultEnvironment>(other);
            assert!(!sm.accept_buyer(deal_id));
            assert!(!sm.accept_provider(deal_id));
            assert!(!sm.complete_storage(deal_id));
